...
var ComponentsAdmin = {
  ...
  _showAdForm: async function(adId) {
    ...
    document.getElementById('app').innerHTML = '<div class="page-fade-in"><div class="settings-page"><div class="settings-card">' +
      ...
      '<div class="form-group"><label class="form-label">广告图片 <span style="font-size:12px;color:var(--text-light);">（建议宽高比约 1:3，图片将铺满广告栏）</span></label><div style="display:flex;gap:8px;align-items:center;"><button type="button" class="btn btn-sm btn-outline" id="ad-upload-img">📁 上传图片</button><input type="file" id="ad-img-file" accept="image/*" style="display:none;"><span id="ad-img-name" style="font-size:13px;">' + (previewUrl ? '已选择' : '未选择') + '</span></div>' +
