...
var ComponentsAdmin = {
  ...
  _showAdForm: async function(adId) {
    ...
    document.getElementById('app').innerHTML = '<div class="page-fade-in"><div class="settings-page"><div class="settings-card">' +
      ...
      '<label class="form-checkbox"><input type="checkbox" class="ad-page-cb" value="chats"' + ((ad && (ad.display_pages || '').split(',').indexOf('chats') >= 0) || !ad ? ' checked' : '') + '> 💬 聊天区</label>' +
      '<label class="form-checkbox"><input type="checkbox" class="ad-page-cb" value="jobs"' + ((ad && (ad.display_pages || '').split(',').indexOf('jobs') >= 0) || !ad ? ' checked' : '') + '> 💼 求职招聘</label></div></div>' +
