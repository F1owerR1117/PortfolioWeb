  ...
  _renderMyPostGrid: function() {
    ...
    grid.innerHTML = this._myPostsList.map(function(pt) {
      ...
      return '<div class="post-card" data-post-id="' + pt.id + '">' +
        ...
        (pt.cover_url ? '' : '📄') + '<div class="post-card-overlay">🔍 查看详情</div></div>' +
        ...
        '<div class="post-card-footer"><div class="post-card-tags">' + (pt.tags ? pt.tags.split(',').map(function(t) { return '<span class="tag">' + escapeHtml(t.trim()) + '</span>'; }).join('') : '') + '</div>' +
        '<div class="stats"><span class="stat">👍 ' + (pt.like_count || 0) + '</span><span class="stat">👁 ' + (pt.views || 0) + '</span></div></div>' +
