...
// GET /api/users/search — search users by username (for perspective switcher)
...
router.get('/users/:id/profile', requireAuth, async (req, res) => {
  try {
    ...
    console.error('[Users] Get public profile error:', err);
  ...
  try {
    ...
    res.json({
      ...
      pagination: {
        ...
        hasMore: offset + limit < (total ? total.count : 0)
      }
    });
  } catch (err) {
    console.error('[Users] Get user posts error:', err);
