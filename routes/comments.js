...
router.get('/posts/:postId/comments', requireAuth, async (req, res) => {
  try {
    ...
    // Verify post exists (not soft-deleted)
...
router.post('/posts/:postId/comments', requireAuth, requireNotBanned, async (req, res) => {
  try {
    ...
    // Verify post exists and is not locked or deleted
    ...
    // XP reward for creating a comment
    ...
    // XP reward: reply → parent comment author, top-level → post author
    if (parent_id) {
      const parentComment = get('SELECT user_id FROM comments WHERE id = ?', [parent_id]);
      if (parentComment && parentComment.user_id !== req.session.userId) {
        addXP(parentComment.user_id, 5);
      }
    } else {
      const postAuthor = get('SELECT created_by FROM posts WHERE id = ?', [postId]);
      if (postAuthor && postAuthor.created_by !== req.session.userId) {
        addXP(postAuthor.created_by, 5);
      }
    }
...
// PUT /api/comments/:id — edit a comment (owner or admin)
...
router.delete('/comments/:id', requireAuth, async (req, res) => {
  try {
    ...
    // Collect all descendant IDs recursively (multi-level cascade)
    ...
    // Delete all descendant comments bottom-up
    // Reverse so children are deleted before parents (avoids FK issues)
    idsToDelete.reverse().forEach(id => {
      if (id !== commentId) run('DELETE FROM comments WHERE id = ?', [id]);
    });
